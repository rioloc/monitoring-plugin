...
const IncidentsChart = ({
  ...
  incidentsData = incidentsData.map((incident) => {
    ...
    const matchedMinTimestamp = matchTimestampMetric(incident, incidentsTimestamps.min_over_time);
    const matchedLastTimestamp = matchTimestampMetric(incident, incidentsTimestamps.last_over_time);

    return {
      ...incident,
      first_timestamp: parseInt(matchedMinTimestamp?.value?.[1] ?? '0'),
      last_timestamp: parseInt(matchedLastTimestamp?.value?.[1] ?? '0'),
    };
  });
                ...
                <ChartVoronoiContainer
                  ...
                  labels={({ datum }) => {
                    ...
                    const endDate = datum.firing
                      ? '---'
                      : dateTimeFormatter(i18n.language).format(new Date(datum.end_date));
