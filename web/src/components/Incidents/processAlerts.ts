...
export function convertToAlerts(
  ...
  const alerts = deduplicatedAlerts
    .map((alert: PrometheusResult): Alert | null => {
      // The alertFirstTimestamp is the first value of the deduplicated interval
      // (computed from the full 15-day data, before any window filtering)
      const alertFirstTimestamp = alert.values[0]?.[0] ?? 0;
      ...
      const chartFirstTimestamp = paddedValues[0][0];
      ...
      return {
        ...
        values: paddedValues,
        alertsStartFiring: chartFirstTimestamp,
